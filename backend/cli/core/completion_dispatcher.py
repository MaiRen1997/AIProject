...
class CompletionDispatcher:
    ...
    def dispatch(self, cli: typer.Typer) -> None:
        ...
        if self.support.is_click_style_instruction(instruction):
            raise SystemExit(
                click_shell_complete(click_command, {}, 'rowen', self.support.complete_env_var, instruction)
            )
        if self.support.is_typer_style_instruction(instruction):
            raise SystemExit(
                typer_shell_complete(click_command, {}, 'rowen', self.support.complete_env_var, instruction)
            )
