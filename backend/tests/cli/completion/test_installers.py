...
def test_build_source_command_uses_shell_runtime_policy() -> None:
    ...
    target_file = Path('/tmp/rowen-completion')

    assert installer.build_source_command(target_file, 'bash') == 'source /tmp/rowen-completion'
    assert installer.build_source_command(target_file, 'zsh') == 'source /tmp/rowen-completion'
    assert (
        installer.build_source_command(target_file, 'fish')
        == 'status --is-interactive; and source /tmp/rowen-completion'
    )
    assert installer.build_source_command(target_file, 'powershell') == '. "/tmp/rowen-completion"'
...
def test_make_bash_completion_script_compatible_wraps_legacy_instructions() -> None:
    ...
    :return: None
    """
    script_text = 'compopt -o dirnames\ncompopt -o default\ncomplete -o nosort -F _rowen_completion rowen'
    ...
    assert 'complete -F _rowen_completion rowen' in compatible_script
    assert 'complete -o nosort -F _rowen_completion rowen 2>/dev/null' in compatible_script
...
def test_render_powershell_completion_script_contains_native_completer() -> None:
    ...
    assert 'Register-ArgumentCompleter -Native -CommandName rowen' in rendered_script
