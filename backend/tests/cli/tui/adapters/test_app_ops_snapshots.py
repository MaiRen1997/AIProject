...
def test_collect_app_page_snapshot_builds_env_config_and_route_sections(
            ...
            return SimpleNamespace(
                payload={
                    ...
                    'runtime': {
                        ...
                        'pythonExecutable': '/opt/conda/envs/rowen-fastapi/bin/python',
                    },
                }
            )
        ...
        if arguments[0:2] == ('completion', 'doctor'):
            return SimpleNamespace(
                payload={
                    ...
                    'recommendedInstallCommand': 'rowen completion install --shell=bash --activate',
                    'shells': {
                        'bash': {
                            'supported': True,
                            'detected': True,
                            'targetFile': '/tmp/rowen.bash',
                            'targetFileExists': True,
                            'rcFile': '/Users/demo/.bashrc',
                            'rcFileExists': True,
                            'autoDiscovery': False,
                            'sourceCommand': 'source /tmp/rowen.bash',
                            'recommendedInstallCommand': 'rowen completion install --shell=bash --activate',
                        },
                        ...
                        'powershell': {
                            ...
                            'detected': False,
                            'autoDiscovery': False,
                            'sourceCommand': '. "/tmp/rowen.ps1"',
                            'recommendedInstallCommand': 'rowen completion install --shell=powershell --activate',
                        },
                    },
                }
            )
    ...
    assert any('source /tmp/rowen.bash' in line for line in snapshot.sections[5].lines)
...
def test_collect_ops_page_snapshot_builds_health_dependency_and_server_sections(
    ...
    assert any('主机名: rowen-node' in line for line in snapshot.sections[5].lines)
