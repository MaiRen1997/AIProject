        ...
        if arguments[0:2] == ('ops', 'server-info'):
            return SimpleNamespace(
                payload={
                    'ok': True,
                    'server': {
                        'sys': {'computerName': 'rowen-node', 'computerIp': '10.0.0.8'},
                        'cpu': {'used': 18.2},
                        'mem': {'usage': 42.5},
                    },
                }
...
def test_collect_configs_page_snapshot_builds_browser_records(
    ...
    assert any('键值: Rowen' in line for line in detail_sections[1].lines)
    assert any('键值: Rowen-Cache' in line for line in detail_sections[2].lines)
    assert snapshot.shared_sections[0].title == '总览判断'
    ...
    assert any('config set site_name Rowen --output=text' in line for line in snapshot.shared_sections[4].lines)
