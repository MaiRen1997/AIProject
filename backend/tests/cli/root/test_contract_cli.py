...
def test_root_help_shows_commands_without_completion_options(
    ...
    assert 'Usage: rowen [OPTIONS] COMMAND [ARGS]...' in completed.stdout
...
def test_completion_show_bash_outputs_completion_script(
    ...
    assert '_rowen_completion()' in completed.stdout
    assert 'if command -v compopt >/dev/null 2>&1; then' in completed.stdout
    assert 'if complete -o nosort -F _rowen_completion rowen 2>/dev/null; then' in completed.stdout
    assert 'complete -F _rowen_completion rowen' in completed.stdout
...
def test_completion_show_powershell_outputs_completion_script(
    ...
    assert completed.returncode == SUCCESS
    assert completed.stderr == ''
    assert 'Register-ArgumentCompleter -Native -CommandName rowen' in completed.stdout
...
def test_bash_completion_protocol_returns_candidates_without_not_supported_error(
    ...
    completed = run_cli_completion_command(comp_words='rowen comp', comp_cword=1)
...
def test_completion_install_json_output_has_stable_contract(
    ...
    target_file = tmp_path / 'rowen.bash'
...
def test_completion_install_powershell_json_output_has_stable_contract(
    ...
    target_file = tmp_path / 'rowen.ps1'
    ...
    assert target_file.exists()
    assert rc_file.exists()
    assert 'Register-ArgumentCompleter -Native -CommandName rowen' in target_file.read_text(encoding='utf-8')
    assert payload['sourceCommand'] in rc_file.read_text(encoding='utf-8')
...
def test_app_run_help_only_exposes_env_option(
    ...
    assert completed.returncode == 0
    assert 'Usage: rowen app run [OPTIONS]' in completed.stdout
